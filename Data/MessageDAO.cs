...
namespace Church_App.Data
{
    internal class MessageDAO
    {
        ...
        public List<MessageModel> FetchSunAM()
        {
            List<MessageModel> returnList = new List<MessageModel>();

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string sqlQuery = "SELECT * FROM Pastoral.Message WHERE DATEPART(HH, ServiceDate) = 10 AND LEN(Video)>2 ORDER BY ServiceDate Desc";
